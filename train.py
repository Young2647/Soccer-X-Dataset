...
from data_loaders.dataset import MotionDataset
...
def train(config, resume, logger):
    common.fixseed(1024)
    ...
    input_feats = (train_data.joint_num+5) * train_data.per_rot_feat   # use the root translation as an extra joint

    model = MotionDiffusion(input_feats, len(train_data.style_set),
                train_data.joint_num+5, train_data.per_rot_feat, 
    ...
    trainer = MotionTrainingPortal(config, model, diffusion, train_dataloader, logger)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='### Generative Soccer Motion Training')
    ...
    parser.add_argument('--device', default=0, type=int, help='which GPU device to use')
    parser.add_argument('--cluster', action='store_true', help='continue training with GPU cluster')
    ...
    common.mkdir(config.save)

    logger = Logger('%s/log.txt' % config.save)
    wandb_mode = 'disabled' if 'debug' in args.name else 'online'
    wandb_writer = wandb.init(project='soccerMDM', name=args.name, config=config, dir=os.path.abspath(config.save), mode=wandb_mode)

    config.device = torch.device(f"cuda:{args.device}" if torch.cuda.is_available() else "cpu")
    with open('%s/config.json' % config.save, 'w') as f:
        f.write(str(config))
    f.close()

    logger.info('\Generative locamotion training with config: \n%s' % config)
    train(config, args.resume, logger)
