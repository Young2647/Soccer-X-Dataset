...
def add_eval_args(parser):
    parser.add_argument('--classifer_path', default=None, type=str, help='The path of the classifier model.')
    


def config_parse(args, mode='train'):
    ...
    if mode == 'test':
        ...
        # trajetory model
        try:
            config.traj_model_path = args.traj_model_path
    ...
    config.trainer.use_loss_ball_acc = True if loss_terms[4] == '1' else False
    config.trainer.use_loss_transition_vel = True if loss_terms[5] == '1' else False
    config.trainer.use_loss_ball_global = True if loss_terms[6] == '1' else False
    config.trainer.load_num = -1
    config.trainer.save_freq = int(config.trainer.epoch // 10)

    config.test.batch_size = int(args.batch_size) if args.batch_size is not None else config.test.batch_size
