...
class ParametricModel:
    ...
    def view_mesh(self, vertex_list: list, fps=60, vid_name='ab', distance_between_subjects=0.8):
        r"""
        ...
        faces = np.concatenate(f_list)
        if verts.shape[0] > 1:
            vo3d.render_sequence_3d(verts, faces, 720, 720, 'offline_output'+ '/' + vid_name+'.mp4', fps, visible=True)
        else:
            vo3d.vis_mesh(verts[0], faces)

    def view_motion(self, pose_list: list, tran_list: list = None, vid_name='output', fps=60, distance_between_subjects=0.8):
        r"""
        ...
        self.view_mesh(verts, fps, distance_between_subjects=distance_between_subjects, vid_name=vid_name)
