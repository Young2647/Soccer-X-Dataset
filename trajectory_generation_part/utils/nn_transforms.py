...
from utils.config import joint_set
...
def smpl_FK(rotations, body_bone, root_position=None, rotation_type='q', order='xyz'):
    b, n, f, _ = rotations.shape
    rotation_type = rotation_type.split('_')[0].lower()
    if rotation_type == 'q':
        norm = torch.norm(rotations, dim=-1, keepdim=True)
        rotations = rotations / norm
    ...
    transforms = transforms.reshape(-1, f, 3, 3)
    global_position = art.math.forward_kinematics(transforms, body_bone.type_as(transforms).expand(transforms.shape[0], -1, -1), joint_set.full_parent)[1]    
    global_position = global_position.reshape(b, n, f, 3)
    if not root_position is None:
        return global_position + root_position.view(b,n,1,3)
    else:
        return global_position
