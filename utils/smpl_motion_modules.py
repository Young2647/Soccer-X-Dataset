...
def on_ground(motion: Motion):
    '''
    make sure the model is always above ground
    '''
    global_pos = motion.positions
...
def root(motion: Motion, given_pos=None, return_pos=False):
    r'''
    make the first frame root position(x,z) back to (0,0)
    '''
    root_init_pos = motion.positions[0, 0, [0, 2]] if given_pos is None else given_pos
    motion.positions[:, :, [0, 2]] -= root_init_pos
...
'''
def extract_forward(motion: Motion, frame_idx, l_s_idx, r_s_idx, l_h_idx, r_h_idx, return_forward=False):
