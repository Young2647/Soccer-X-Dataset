...
class MotionDiffusion(nn.Module):
    def __init__(self, input_feats, nstyles, njoints, nfeats, rot_req, clip_len,
                 ...
                 arch='trans_enc', cond_mask_prob=0, device=None, mode='train'):
        super().__init__()
    ...
    def interface(self, x, timesteps, y=None, cfg_weight=None):
        """
        ...
        if self.mode == 'train':
            ...
            x_0 = self.forward(x, timesteps, past_motion, traj_pose, traj_trans, style_idx)
        elif self.mode == 'test':
            if cfg_weight != None and cfg_weight != 1.0:
                empty_past_motion = torch.zeros_like(past_motion).to(past_motion.device)
                x_0_uncond = self.forward(x, timesteps, empty_past_motion, traj_pose, traj_trans, style_idx)
                x_0 = self.forward(x, timesteps, past_motion, traj_pose, traj_trans, style_idx)
                x_0 = x_0_uncond + cfg_weight*(x_0-x_0_uncond)
            else:
                x_0 = self.forward(x, timesteps, past_motion, traj_pose, traj_trans, style_idx)
        return x_0
